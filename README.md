...
About 10–12 minutes. Everything below works — there are no dead ends.
...
### 5 · The thing that sells it over a Google Doc — Version history + diff
...
**pinned in a side panel** — the ask and the reason sit beside the document while you make the
...
same action that makes the change, so the log can't drift from reality. Click **View diff** on a
...
re-consent**. That's the blast radius made concrete.
...
- **Approval before publish.** Open *Data Analytics — NSDC Certified* (Pending approval). Its draft
  ...
  **Approve draft** button appears and Publish unlocks. Four-eyes, demoed live.
...
- **Clause numbers are derived, never stored.** `numberBlocks()` walks the block array on every
  render. That's why insert/delete/indent renumbers correctly with nothing to keep in sync.
